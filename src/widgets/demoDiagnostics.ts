/*
  Heuristic analysis and diagnostics for action logs. This module defines a set of heuristic rules that can be applied to the action logs generated by the demo streams to identify potentially problematic or interesting patterns. It also provides functionality to minimize the action log to the relevant actions that triggered the heuristic findings, along with their dependencies.
*/
...
export const HEURISTIC_RULES: HeuristicRule[] = [
  ...
  },
  {
    ...
    severity: "weird",
    evaluate: (action) => {
      const messagesCreated = action.added.filter(
        (m) => m.text.trim() === "",
      ).length;
      return {
    ...
    const collectEntityIdsFromInput = (
      ...
      Object.values(input).forEach((instances) => {
        instances.forEach((instance) => {
          if (!isObjectInstance(instance)) return;
  ...
  const collectDependencyActionIds = (action: DemoActionImpact) => {
    ...
    relatedEntityIds?.forEach((entityId) => {
      ...
      relatedActions.forEach((relatedActionId) => {
        ...
        if (!relatedAction || relatedAction.order >= action.order) return;
