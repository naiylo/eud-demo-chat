...
export type PropertyDefinition = {
  name: string;
  type: "string" | "number" | "boolean" | "date" | "object" | "persona" | "id" | "reference";
...
function randomProperty(propDef: PropertyDefinition, personas: string[], currentState: Record<string, ObjectInstance[]>, rng: () => number): unknown {
  switch (propDef.type) {
    case "string":
    ...
    case "object":
      if (propDef.schema) {
        ...
        for (const subPropDef of Object.values(propDef.schema)) {
          obj[subPropDef.name] = randomProperty(subPropDef, personas, currentState, rng);
        }
        return obj;
      }
      return {};
    ...
    case "reference":
      if (propDef.referenceSchema) {
        ...
        if (targetObjects && targetObjects.length > 0) {
          ...
          console.warn(`No target objects available for reference to schema ${propDef.referenceSchema}`);
          return null;
        }
      }
      return null;
...
export function randomObjectInstance(schema: ObjectSchema, id: string, personas: string[], currentState: Record<string, ObjectInstance[]>, rng: () => number): ObjectInstance {
  const properties: Record<string, unknown> = {};
  for (const propDef of schema.properties) {
    if (propDef.linkedTo) {
        ...
        while (layers.length > 1) {
          ...
          const nextSchemaName: string | undefined = currentSchema!.properties.find(prop => prop.name === linkedProperty && prop.type === "reference")?.referenceSchema;
        ...
        properties[propDef.name] = Array.from({ length: arrLen }, () => randomProperty(propDef, personas, currentState, rng));
      } else {
        properties[propDef.name] = randomProperty(propDef, personas, currentState, rng);
      }
  }
  return {
    id,
