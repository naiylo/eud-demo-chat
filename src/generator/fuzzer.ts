/*
    This module contains a fuzzer that generates random action logs based on the defined actions and their pre- and post-conditions. It ensures that the generated logs are valid according to the constraints defined in the actions.
*/
...
async function generateAction(
  ...
  for (const def of action.inputDefinition) {
    ...
    if (
      def.uniqueInstance ||
      !objects[def.schema.name] ||
      objects[def.schema.name].length === 0
    ) {
      const amount = Math.floor(
        rng() * ((def.maxCount ?? 1) - (def.minCount ?? 0) + 1) +
          (def.minCount ?? 0),
      );
      for (let i = 0; i < amount; i++) {
        input[def.name].push(
          await createNextObjectInstance(def.schema, personas, rng),
        );
      }
    } else {
      const existingCount = objects[def.schema.name].length;
      const amount = Math.floor(
        rng() * ((def.maxCount ?? existingCount) - (def.minCount ?? 0) + 1) +
          (def.minCount ?? 0),
      );
      for (let i = 0; i < amount; i++) {
        const objInstance =
          objects[def.schema.name][Math.floor(rng() * existingCount)];
...
export function analyzeDependencies(actions: Action[]): Action[] | undefined {
  ...
  for (const action of actions) {
    dependencies[action.name] = [
      ...new Set(
        action.inputDefinition.flatMap((ref, _, others) =>
          ref.schema.properties
            .filter(
              (p) =>
                p.type === "reference" &&
                !others.find((o) => o.schema.name === p.referenceSchema),
            )
            .map((p) => p.referenceSchema!),
        ),
      ),
    ];
  ...
  while (backlog.length > 0) {
    ...
    for (const action of backlog) {
      if (
        dependencies[action.name].every((dep) =>
          result.find((a) => produces[a.name].includes(dep)),
        )
      ) {
        result.push(action);
      } else {
